...
function net.forward(input)
     ...
     norm_input = net.input_normalization:forward(input:view(num_samples*feat_len,1366))
...
function net.backward(input, gradOutput)
  ...
  assert(norm_input ~= nil, "forward not called")

  gradOutput = gradOutput:view(gradOutput:size(1), gradOutput:size(2), 1, gradOutput:size(3))
  local grad_norm_input = net.cnn:backward(norm_input, gradOutput)
  local gradInput = net.input_normalization:backward(input:view(num_samples*feat_len,1366),grad_norm_input:view(num_samples*feat_len,1366))
  gradInput = gradInput:view(num_samples,1,feat_len,1366)
