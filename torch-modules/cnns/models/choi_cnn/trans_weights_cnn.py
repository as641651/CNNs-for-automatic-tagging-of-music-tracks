...
with h5py.File('music_tagger_cnn_weights_theano.h5','r') as hf:
   for k in hf.keys():
      ...
      if(k == "conv2"):
         net["conv2"] = nn.SpatialConvolutionMM(32,128,3,3,1,1)
         addWeights(layer,k,hf)
         transferW(net["conv2"].weight,layer["conv2"]["conv2_W"].reshape(128,288)) 
         transferB(net["conv2"].bias,layer["conv2"]["conv2_b"]) 
      if(k == "conv3"):
         net["conv3"] = nn.SpatialConvolutionMM(128,128,3,3,1,1)
         addWeights(layer,k,hf)
         transferW(net["conv3"].weight,layer["conv3"]["conv3_W"].reshape(128,1152)) 
         transferB(net["conv3"].bias,layer["conv3"]["conv3_b"]) 
      if(k == "conv4"):
         net["conv4"] = nn.SpatialConvolutionMM(128,192,3,3,1,1)
         addWeights(layer,k,hf)
         transferW(net["conv4"].weight,layer["conv4"]["conv4_W"].reshape(192,1152)) 
         transferB(net["conv4"].bias,layer["conv4"]["conv4_b"]) 
      if(k == "conv5"):
         net["conv5"] = nn.SpatialConvolutionMM(192,256,3,3,1,1)
         addWeights(layer,k,hf)
         transferW(net["conv5"].weight,layer["conv5"]["conv5_W"].reshape(256,1728)) 
         transferB(net["conv5"].bias,layer["conv5"]["conv5_b"])     

PyTorchAug.save("choi_cnn.t7",net)
