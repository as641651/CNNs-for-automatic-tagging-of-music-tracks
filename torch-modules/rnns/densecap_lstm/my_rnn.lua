...
function get_rnn()
  ...
  for i = 1, net.opt.rnn_layers do
    ...
    rnn:add(nn.LSTM(input_dim, net.opt.rnn_hidden_size))
    if net.opt.dropout > 0 then
      rnn:add(nn.Dropout(net.opt.dropout))
    end
  end

  rnn:add(rnn_view_in)
  rnn:add(nn.Linear(net.opt.rnn_hidden_size, net.opt.classifier_vocab_size+1))
  rnn:add(rnn_view_out)
