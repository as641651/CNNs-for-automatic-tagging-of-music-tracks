...
with h5py.File('music_tagger_crnn_weights_theano.h5','r') as hf:
   for k in hf.keys():
      k = str(k)
      print(k)
      if(k == 'bn_0_freq'):
         net["bn_0_freq"] = nn.BatchNormalization(1440)
      ...
      if(k == 'bn1'):
         net["bn1"] = nn.SpatialBatchNormalization(64)
         addWeights(layer,k,hf)
      ...
      if(k == 'bn4'):
         net["bn4"] = nn.SpatialBatchNormalization(128)
         addWeights(layer,k,hf)
      ...
      if(k == "conv1"):
         net["conv1"] = nn.SpatialConvolutionMM(1,64,3,3)
         addWeights(layer,k,hf)
         transferW(net["conv1"].weight,layer["conv1"]["conv1_W"].reshape(64,9)) 
         transferB(net["conv1"].bias,layer["conv1"]["conv1_b"]) 
      if(k == "conv2"):
         net["conv2"] = nn.SpatialConvolutionMM(64,128,3,3)
         addWeights(layer,k,hf)
         transferW(net["conv2"].weight,layer["conv2"]["conv2_W"].reshape(128,576)) 
      ...
      if(k == "conv4"):
         net["conv4"] = nn.SpatialConvolutionMM(128,128,3,3)
         addWeights(layer,k,hf)
         transferW(net["conv4"].weight,layer["conv4"]["conv4_W"].reshape(128,1152)) 
